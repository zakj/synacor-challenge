    ...
    with open("challenge.asm", "w") as f:
        while cur < len(memory):
            ...
            if memory[cur] in OPS:
                ...
                f.write(
                    " ".join([op] + [str(a) for a in memory[cur + 1 : cur + 1 + args]])
                    + "\n"
                )
                cur += 1 + args
            else:
...
while True:
    ...
    if op == "halt":
        ...
        registers[memory[cur + 1] - 32768] = (
            1 if value(memory[cur + 2]) == value(memory[cur + 3]) else 0
        )
        cur += args + 1
    elif op == "gt":
        registers[memory[cur + 1] - 32768] = (
        ...
        registers[memory[cur + 1] - 32768] = (
            value(memory[cur + 2]) + value(memory[cur + 3])
        ) % 32768
        cur += args + 1
    elif op == "mult":
        registers[memory[cur + 1] - 32768] = (
        ...
        registers[memory[cur + 1] - 32768] = value(memory[cur + 2]) % value(
            memory[cur + 3]
        )
        cur += args + 1
    elif op == "and":
